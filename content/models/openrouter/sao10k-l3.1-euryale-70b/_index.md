---
title: "Sao10K Llama 3.1 Euryale 70B v2.2"
model: true
date: 2026-09-12T00:00:00Z
reasoning: 80
description: "Euryale L3.1 70B v2.2 is a model focused on creative roleplay from [Sao10k](https://ko-fi.com/sao10k). It is the successor of [Euryale L3 70B v2.1](/models/sao10k/l3-euryale-70b)."
specs:
  vendor: "Sao10K"
  category: "其他"
  released: "2025"
  context: "131K tokens"
  price_input: "$0.0009/K"
  price_output: "$0.0009/K"
  modalities: "text->text"
  openrouter_id: "sao10k/l3.1-euryale-70b"

  scores:
    reasoning: 80
    coding: 85
    chinese: 70
    longtext: 85
---

## 模型概况

Llama 3.1 Euryale 70B v2.2 是由 Sao10K 提供、通过 OpenRouter 平台聚合的模型。该模型在 OpenRouter 上提供统一的 API 接口。

### 核心参数

| 维度 | 数值 |
|------|------|
| 厂商 | Sao10K |
| OpenRouter ID | `sao10k/l3.1-euryale-70b` |
| 上下文窗口 | 131K tokens |
| 输入价格 | $0.0000/M |
| 输出价格 | $0.0000/M |
| 模态 | text->text |

## OpenRouter 平台数据

Euryale L3.1 70B v2.2 is a model focused on creative roleplay from [Sao10k](https://ko-fi.com/sao10k). It is the successor of [Euryale L3 70B v2.1](/models/sao10k/l3-euryale-70b).

### 定价信息

| 项目 | 价格 |
|------|------|
| 输入 | $0.0000/M |
| 输出 | $0.0000/M |

### 支持的参数

该模型支持以下参数：
- 温度调节
- Top-P / Top-K 采样
- 流式输出
- 工具调用（如支持）

## 适用场景

- 长文本对话、文档总结
- 常规文本生成与对话

## 来源

- [OpenRouter Models API](https://openrouter.ai/models)
