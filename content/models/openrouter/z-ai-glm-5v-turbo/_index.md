---
title: "智谱 AI GLM 5V Turbo"
model: true
description: "GLM-5V-Turbo is Z.ai’s first native multimodal agent foundation model, built for vision-based coding and agent-driven tasks. It natively handles image, video, and text inputs, excels at long-horizon p"
specs:
  vendor: "智谱 AI"
  category: "国内"
  released: "2025"
  context: "202K tokens"
  price_input: "$0.0012/K"
  price_output: "$0.0040/K"
  modalities: "text+image+video->text"
  openrouter_id: "z-ai/glm-5v-turbo"

  scores:
    reasoning: 80
    coding: 85
    chinese: 70
    longtext: 85
---

## 模型概况

GLM 5V Turbo 是由 智谱 AI 提供、通过 OpenRouter 平台聚合的模型。该模型在 OpenRouter 上提供统一的 API 接口。

### 核心参数

| 维度 | 数值 |
|------|------|
| 厂商 | 智谱 AI |
| OpenRouter ID | `z-ai/glm-5v-turbo` |
| 上下文窗口 | 202K tokens |
| 输入价格 | $0.0000/M |
| 输出价格 | $0.0000/M |
| 模态 | text+image+video->text |

## OpenRouter 平台数据

GLM-5V-Turbo is Z.ai’s first native multimodal agent foundation model, built for vision-based coding and agent-driven tasks. It natively handles image, video, and text inputs, excels at long-horizon planning, complex coding,...

### 定价信息

| 项目 | 价格 |
|------|------|
| 输入 | $0.0000/M |
| 输出 | $0.0000/M |

### 支持的参数

该模型支持以下参数：
- 温度调节
- Top-P / Top-K 采样
- 流式输出
- 工具调用（如支持）

## 适用场景

- 图像理解、视觉问答
- 长文本对话、文档总结
- 常规文本生成与对话

## 来源

- [OpenRouter Models API](https://openrouter.ai/models)
